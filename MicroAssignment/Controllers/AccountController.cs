...
namespace MicroAssignment.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult AccountLogin(string Username, string Password)
        {
            ...
            {
                TempData["error"] = "Sorry! The username colum can not be empty";
                return RedirectToAction("Register");
            }
            else if (Password == null || Password=="".Trim())
            {
                TempData["error"] = "Sorry! The password colum can not be empty";
                return RedirectToAction("Register");
            }
            else
            {
                if (ModelState.IsValid && WebSecurity.Login(Username, Password))
                {
                    return RedirectToAction("Index", "Profile");
                }
        ...
        {
            ViewBag.Count = db.UserProfiles.Count();
            var department = db.Departments.OrderBy(x => x.DepartmentName);
            ViewBag.DepartmentId = new SelectList(department, "DepartmentId", "DepartmentInfo");
            return View();
        }
        ...
        public ActionResult Register()
        {
            ...
            ViewBag.LevelId = new SelectList(db.Levels, "LevelId", "FullLevel");
            return View();
        }
        ...
        public ActionResult Search(string SearchString)
        {
            if (SearchString == "")
            {
                TempData["error"] = "Enter your username";
                return RedirectToAction("ResetPassword");
            }

            var userInfo = db.UserProfiles.FirstOrDefault(x => x.UserName.ToUpper() == SearchString.ToUpper());
            if (userInfo == null)
            {
                TempData["error"] = "Username not found";
                return RedirectToAction("ResetPassword");
            }
            return RedirectToAction("ResetPassword", new { id = userInfo.UserId });
        }
        ...
        public ActionResult ResetPassword(int? id)
        {
            ...
            if (id != null)
            {
                ...
                ViewBag.Username = userDetails.FullName;
                
                ViewBag.Email = userDetails.EmailAddress;
                ViewBag.Image = userDetails.ImageUrl;
                 
                if (userDetails.PhoneNumber == null)
                {
                    userDetails.PhoneNumber = "080";
                }

                if (userDetails.PhoneNumber != null && userDetails.PhoneNumber.Length >= 11)
                {
                    var phoneno = "xxxxxxxx" + userDetails.PhoneNumber.Substring(8, 3);
        ...
        public ActionResult ResetPassword(ResetPasswordModel model, int? id)
        {
            try
            {
                ...
                var usermodel = new MicroContext();

                string emailAddress = usermodel.UserProfiles.SingleOrDefault(x => x.UserName == userDetails.UserName).EmailAddress;
                if (!string.IsNullOrEmpty(emailAddress))
                {
                    string confirmationToken = WebSecurity.GeneratePasswordResetToken(userDetails.UserName);

                    dynamic email = new Email("ChangePasswordEmail");
                    email.To = userDetails.EmailAddress;
                    email.username = userDetails.UserName;
                    ...
                    return RedirectToAction("ChangePassword", new { id = userDetails.UserId });
                }
            }
            catch (Exception e)
            {
            ...
            TempData["Report"] = "Password Reset failed. The username does not exist.";
            return RedirectToAction("ResetPassword");
        }
